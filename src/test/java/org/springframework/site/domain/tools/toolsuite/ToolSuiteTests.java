...
public class ToolSuiteTests {
	...
	public void testPreferredDownloadLinks() throws Exception {
		ToolSuite toolSuite = buildToolSuite();
	...
	private ToolSuite buildToolSuite() {
		...
		platforms.put("mac", new Platform("Mac", "3.0.0.RELEASE", macEclipseVersions));
		...
		platforms.put("windows", new Platform("Windows", "3.0.0.RELEASE", Collections.singletonList(new EclipseVersion("4.3", winArchitectures))));
		...
		platforms.put("linux", new Platform("Linux", "3.0.0.RELEASE", Collections.singletonList(new EclipseVersion("4.3", linuxArchitectures))));

		List<UpdateSiteArchive> archives = Collections.emptyList();
		return new ToolSuite(platforms, archives);
	}
}
