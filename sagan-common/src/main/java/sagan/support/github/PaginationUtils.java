...
public final class PaginationUtils {
	...
	/**
	 * Consume a pageable resource from Github but reading each page of data then navigating to the next entry
	 * via hypermedia in the {@literal Link} header.
	 *
	 * @see https://developer.github.com/v3/#link-header
	 *
	 * @param githubRequest - {@link Function} that performs a Github request, in the form of url -> github.request(url)
	 * @param objectMapper - {@link ObjectMapper} used to deserialize body of github response
	 * @param clazz - {@link Class} of the collection
	 * @return {@link List} of {@literal T} objects, based on iterating over multiple pages.
	 */
	static public <T> List<T> readResults(Function<String, ResponseEntity<String>> githubRequest, String requestUrl, ObjectMapper objectMapper, Class<T> clazz) {
		...
		while (!url.equals("")) {
			...
			// Make a github request, but also be sure to grab any headers.
			ResponseEntity<String> pageOfJson = githubRequest.apply(url);

			// Deserialize the JSON into a {@literal List<T>}, then add it to the final collection
			try {
