...
import sagan.blog.service.SummaryExtractor;
...
public class SummaryExtractorTests {
    ...
    private SummaryExtractor extractor = new SummaryExtractor();
    ...
    public void extractBasicSummary() {
        ...
        assertThat(extractor.extract(content, 50), is(expected));
    }

    @Test
    public void extractNestedTagSummary() {
        String content = "<p>We?re pleased to announce that, after a long period of internal incubation, we?re releasing a foundational framework for asynchronous applications on the JVM which we?re calling <em>Reactor</em>. It provides abstractions for Java, Groovy and other JVM languages to make building event and data-driven applications easier. It?s also really fast. On modest hardware, it's possible to process over 15,000,000 events per second with the fastest non-blocking <code>Dispatcher</code>. Other dispatchers are available to provide the developer with a range of choices from thread-pool style, long-running task execution to non-blocking, high-volume task dispatching. The GitHub repo is here <a href=\"https://github.com/reactor/reactor\"></a><a href=\"https://github.com/reactor/reactor\">https://github.com/reactor/reactor</a>.</p>\n" +
                "\n" +
        ...
        String expected = "<p>We?re pleased to announce that, after a long period of internal incubation, we?re releasing a foundational framework for asynchronous applications on the JVM which we?re calling <em>Reactor</em>. It provides abstractions for Java, Groovy and other JVM languages to make building event and data-driven applications easier. It?s also really fast. On modest hardware, it's possible to process over 15,000,000 events per second with the fastest non-blocking <code>Dispatcher</code>. Other dispatchers are available to provide the developer with a range of choices from thread-pool style, long-running task execution to non-blocking, high-volume task dispatching. The GitHub repo is here <a href=\"https://github.com/reactor/reactor\"></a><a href=\"https://github.com/reactor/reactor\">https://github.com/reactor/reactor</a>.</p>\n";

        assertThat(extractor.extract(content, 500), is(expected));
    }
